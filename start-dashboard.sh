...
"$TMUX" split-window -h -t "$SESSION:dashboard" "cd '$ROOT' && exec ./monitor_system.sh 10"
"$TMUX" split-window -h -t "$SESSION:dashboard" "cd '$ROOT' && exec ./monitor_network_io.sh '$INTERFACE' 10"
"$TMUX" split-window -h -t "$SESSION:dashboard" "cd '$ROOT' && exec ./monitor_disk_io.sh '$DEVICE' 10"
"$TMUX" split-window -h -t "$SESSION:dashboard" "cd '$ROOT' && exec ./monitor_http_status.sh"
"$TMUX" split-window -h -t "$SESSION:dashboard" "cd '$ROOT' && exec ./monitor_torrent_status.sh"
